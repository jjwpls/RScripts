# The following R-script is motivated by exercise 7 of Chapter 8 of "Introduction of Statistical Learning" by
# Gareth James et al. (2013). 
#
# The codes compare the testing errors (not out-of-bag errors) from the bagging and random 
...
# Build a test set with the actual data value in that set
...
for (i in 25:400) {
  ...
  mse_3[i-24] = mean((yhat.rf-test)^2)
}
...
for (i in 25:400) {
  ...
  yhat.rf = predict(rf.boston,newdata=Boston[-train,])
  mse_5[i-24] = mean((yhat.rf-test)^2)
}
...
for (i in 25:400) {
  ...
  yhat.rf = predict(rf.boston,newdata=Boston[-train,])
  mse_7[i-24] = mean((yhat.rf-test)^2)
}
...
for (i in 25:400) {
  ...
  yhat.rf = predict(rf.boston,newdata=Boston[-train,])
  mse_9[i-24] = mean((yhat.rf-test)^2)
}
...
for (i in 25:400) {
  ...
  yhat.rf = predict(rf.boston,newdata=Boston[-train,])
  mse_bag[i-24] = mean((yhat.rf-test)^2)
}

ntree = 25:400

# The graph will be uploaded to github as well.
png(file = "random_forests_vs_bagging.github.png")
